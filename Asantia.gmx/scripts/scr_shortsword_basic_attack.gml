...
if step=0
{
 ...
 weaponII.image_yscale=-1
 weaponII.image_angle=0
}
...
{
 y=obj_player.y+3
 x=obj_player.x+(6*image_xscale)
 weaponII.x=obj_player.y+3
 weaponII.x=obj_player.x-(6*image_xscale)
}

if step<37
{
 image_angle=(image_xscale*-1)*(step*5)
 weaponII.image_angle=(image_xscale*-1)*(step*5)
 
 blur(0.02,0.2,c_white,false)
 with (weaponII) {blur(0.02,0.2,c_white,false)}
}
