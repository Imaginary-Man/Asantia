...
{
    if (any(frac(IN.Texcoord*resolution1.xy)<float2(0.5, 0.5))) {
    ...
    float3 P0 = tex2D(gm_BaseTexture, IN.Texcoord     -g1    -g2).xyz;
    float3 P1 = tex2D(gm_BaseTexture, IN.Texcoord +2.0*g1    -g2).xyz;
    float3 P2 = tex2D(gm_BaseTexture, IN.Texcoord     -g1+2.0*g2).xyz;
    float3 P3 = tex2D(gm_BaseTexture, IN.Texcoord +2.0*g1+2.0*g2).xyz;
    
    float3  B = tex2D(gm_BaseTexture, IN.Texcoord    -g2).xyz;
    float3  C = tex2D(gm_BaseTexture, IN.Texcoord +g1-g2).xyz;
    float3  D = tex2D(gm_BaseTexture, IN.Texcoord -g1   ).xyz;
    float3  E = tex2D(gm_BaseTexture, IN.Texcoord       ).xyz;
    float3  F = tex2D(gm_BaseTexture, IN.Texcoord +g1   ).xyz;
    float3  G = tex2D(gm_BaseTexture, IN.Texcoord -g1+g2).xyz;
    float3  H = tex2D(gm_BaseTexture, IN.Texcoord    +g2).xyz;
    float3  I = tex2D(gm_BaseTexture, IN.Texcoord +g1+g2).xyz;
    
    float3 F4 = tex2D(gm_BaseTexture,IN.Texcoord    +2.0*g1   ).xyz;
    float3 I4 = tex2D(gm_BaseTexture,IN.Texcoord +g2+2.0*g1   ).xyz;
    float3 H5 = tex2D(gm_BaseTexture,IN.Texcoord +2.0*g2      ).xyz;
    float3 I5 = tex2D(gm_BaseTexture,IN.Texcoord +2.0*g2+g1   ).xyz;
