...
const static float XBR_EDGE_STR = 1.0;
const static float XBR_WEIGHT = 1.0;
...
{
    float2 dir = frac(IN.Texcoord*resolution1/2.0) - float2(0.5,0.5);
     if ((dir.x*dir.y)>0.0) {
        OUT.Color = tex2D(gm_BaseTexture, IN.Texcoord);
        return;
     }
    
    float2 g1 = float2(1.0/resolution1.x, 0.0);
    float2 g2 = float2(0.0, 1.0/resolution1.y);
    
    float3 P0 = tex2D(gm_BaseTexture, IN.Texcoord -3.0*g1       ).xyz;
    float3 P1 = tex2D(gm_BaseTexture, IN.Texcoord        -3.0*g2).xyz;
    float3 P2 = tex2D(gm_BaseTexture, IN.Texcoord        +3.0*g2).xyz;
    float3 P3 = tex2D(gm_BaseTexture, IN.Texcoord +3.0*g1       ).xyz;
    
    float3 B = tex2D(gm_BaseTexture, IN.Texcoord -2.0*g1     -g2).xyz;
    float3 C = tex2D(gm_BaseTexture, IN.Texcoord     -g1 -2.0*g2).xyz;
    float3 D = tex2D(gm_BaseTexture, IN.Texcoord -2.0*g1     +g2).xyz;
    float3 E = tex2D(gm_BaseTexture, IN.Texcoord     -g1        ).xyz;
    float3 F = tex2D(gm_BaseTexture, IN.Texcoord             -g2).xyz;
    float3 G = tex2D(gm_BaseTexture, IN.Texcoord     -g1 +2.0*g2).xyz;
    float3 H = tex2D(gm_BaseTexture, IN.Texcoord             +g2).xyz;
    float3 I = tex2D(gm_BaseTexture, IN.Texcoord     +g1        ).xyz;
    
    float3 F4 = tex2D(gm_BaseTexture,IN.Texcoord     +g1 -2.0*g2).xyz;
    float3 I4 = tex2D(gm_BaseTexture,IN.Texcoord +2.0*g1     -g2).xyz;
    float3 H5 = tex2D(gm_BaseTexture,IN.Texcoord     +g1 +2.0*g2).xyz;
    float3 I5 = tex2D(gm_BaseTexture,IN.Texcoord +2.0*g1     +g2).xyz;
